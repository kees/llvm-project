...
``__builtin_is_lvalue`` returns true when the argument is an assignable lvalue.
...
.. code-block:: c
  ...
  bool __builtin_is_lvalue(T expr)
...
.. code-block:: c
  ...
  #define __force_lvalue_expr(x)  \
          __builtin_choose_expr(__builtin_is_lvalue(x), x, (void *){ NULL })
  ...
  #define __free_and_maybe_null(__how, x)                         \
          __builtin_choose_expr(__builtin_is_lvalue(x),           \
...
function calls can be ignored. This will see through casts, though, so
any macros taking the address of the lvalue will need to avoid having
casted arguments.
